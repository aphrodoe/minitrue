...
func hashNodeInfo(node *models.NodeInfo) string {
	// We hash the crucial state fields. We omit LastHeartbeat because it updates constantly
	// and would cause the root hash to change every second even if the cluster status is identical.
	data := fmt.Sprintf("%s|%s|%s|%d|%d", node.ID, node.Address, node.Status, node.HTTPPort, node.MQTTPort)
