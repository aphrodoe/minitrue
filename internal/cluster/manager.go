...
func (cm *ClusterManager) Initialize(localNode *models.NodeInfo, tcpPort int, seedNodes []string) error {
	...
	if cm.hashRing == nil {
		cm.hashRing = NewConsistentHashRing(150)
	...
	cm.gossipProtocol = NewGossipProtocol(
		localNode,
		2*time.Second,
		networkClient,
		3,
		...
		if seedAddr != "" {
			go func(addr string) {
				if err := cm.gossipProtocol.AddSeedNode(addr); err != nil {
					log.Printf("[Cluster] Failed to connect to peer node %s: %v", addr, err)
				} else {
					log.Printf("[Cluster] Connected to peer node %s", addr)
				}
			}(seedAddr)
		}
	}
...
func (cm *ClusterManager) GetGossipProtocol() *GossipProtocol {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
	return cm.gossipProtocol
}

func (cm *ClusterManager) GetHashRing() *ConsistentHashRing {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
	return cm.hashRing
}
