...
func (se *StorageEngine) Write(records []models.Record) error {
	...
	writeColumn := func(columnName string, data []byte) error {
		if _, err := file.Write(data); err != nil {
			return fmt.Errorf("failed to write %s column: %w", columnName, err)
		}
		if _, err := checksum.Write(data); err != nil {
			return fmt.Errorf("failed to checksum %s column: %w", columnName, err)
		}
		return nil
	}
	...
	if err := writeColumn("timestamp", timestampData); err != nil {
		return err
	}
...
func (se *StorageEngine) buildFooter(timestampOffset, timestampSize,
	...
	recordCount int, dataChecksum uint32) []byte {
	...
	binary.LittleEndian.PutUint32(checksumBuf, dataChecksum)
	footer = append(footer, checksumBuf...)
...
func (se *StorageEngine) Read() ([]models.Record, error) {
	...
	if footerSize == 0 || footerStart < HeaderSize || footerStart > footerSizeOffset {
		return nil, fmt.Errorf("%w: invalid footer size", ErrCorruptSegment)
	}

	footer := data[footerStart:footerSizeOffset]
	if formatVersion >= 3 {
		if len(footer) < 12 {
			return nil, fmt.Errorf("%w: footer too small for checksum", ErrCorruptSegment)
		}
		expectedChecksum := binary.LittleEndian.Uint32(footer[len(footer)-4:])
		actualChecksum := crc32.ChecksumIEEE(data[HeaderSize:footerStart])
		if actualChecksum != expectedChecksum {
			return nil, fmt.Errorf("%w: column data checksum mismatch (expected %08x, got %08x)", ErrCorruptSegment, expectedChecksum, actualChecksum)
		}
		footer = footer[:len(footer)-4]
	}
	if len(footer) < 8 {
		return nil, fmt.Errorf("%w: footer too small", ErrCorruptSegment)
	}
