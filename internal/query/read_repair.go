...
func (s *Service) repairWithPeer(peerID, deviceID, metric string) {
	...
	// Out of sync. If we have fewer records, queue a sync task to fetch from peer.
	if localCount < remoteDigest.Count {
		log.Printf("[ReadRepair] Node %s out of sync for %s (local=%d, remote=%d). Enqueueing...", s.nodeID, seriesKey, localCount, remoteDigest.Count)
		s.syncManager.Enqueue(peerID, deviceID, metric)
	}
}
...
func (s *Service) syncFromPeer(peerID, deviceID, metric string) {
	...
	for _, record := range syncResp.Records {
		s.store.PersistReplica(models.DataPoint{
