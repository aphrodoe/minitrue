...
func (s *Service) checkPeerKeysForMigration(peerID string, peerPort int) {
	...
	for _, key := range keysResp.Keys {
		...
		if nodes[0] == s.nodeID || nodes[1] == s.nodeID {
			// Fast path: let the read-repair pull it. Or we explicitly enqueue it here.
			// Using Enqueue ensures we get it as soon as a worker is free.
			log.Printf("[Migration] Node %s now owns %s (previously checked from %s), enqueueing data pull...", s.nodeID, key, peerID)
