...
const QueryForm = ({ onSubmit, loading, onClearResults }) => {
  ...
  const fetchKeys = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch keys from cluster:', err);
    } finally {
  ...
  const handleDeleteAllData = () => {
    if (!formData.device_id || !formData.metric_name) {
      setAlertConfig({
        ...
        message: 'Please choose both a Device ID and Metric Name from the dropdowns before executing the delete operation.',
        type: 'warning',
        showConfirm: false
      });
      return;
    ...
    setAlertConfig({
      ...
      message: `Are you sure you want to permanently delete all time-series data for:\n\nDevice: ${formatDeviceId(formData.device_id)}\nMetric: ${formatMetricName(formData.metric_name)}\n\nThis action is irreversible and cannot be undone!`,
      type: 'error',
      showConfirm: true,
  ...
  const executeDeleteData = async () => {
    try {
      const response = await fetchFromCluster('/delete', {
        method: 'POST',
        headers: {
      ...
      setAlertConfig({
        ...
        message: `Successfully wiped all telemetry records for device "${formatDeviceId(formData.device_id)}" and metric "${formatMetricName(formData.metric_name)}" from both memory cache and Parquet disk blocks.`,
        type: 'success',
        showConfirm: false
      });
      ...
      setAlertConfig({
        ...
        message: `Failed to execute delete operations: ${err.message || 'Unknown network error occurred.'}`,
        type: 'error',
        showConfirm: false
      });
    ...
    <div className="query-form-container">
        ...
        <div className="form-group">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button 
              ...
              title="Refresh available devices and metrics from the cluster"
            >
              {loadingKeys ? '↻...' : '↻ Refresh'}
            </button>
          </div>
      ...
      {alertConfig && (
        <div className="alert-overlay">
          <div className={`alert-card alert-${alertConfig.type}`}>
            <div className="alert-icon">
              {alertConfig.type === 'success' && '✅'}
              {alertConfig.type === 'error' && '🛑'}
              {alertConfig.type === 'warning' && '⚠️'}
              {alertConfig.type === 'info' && 'ℹ️'}
            </div>
