...
import { getClusterWebSocketUrls } from '../clusterClient';
...
const RealTimeMonitor = () => {
  ...
  const connectWebSocket = () => {
    const urls = getClusterWebSocketUrls('/ws');
    const ws = new WebSocket(urls[wsUrlIndexRef.current % urls.length]);
    let opened = false;
    ...
    ws.onclose = () => {
      ...
      if (!opened) {
        ...
        wsUrlIndexRef.current = (wsUrlIndexRef.current + 1) % urls.length;
      }
